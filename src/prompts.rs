//! Intake prompt catalogue + local rendering.
//!
...
impl Registry {
    ...
    fn load<P: Serialize>(
        ...
        let file = self
            .0
            .get(name)
            .ok_or_else(|| IntakeError::ai(format!("prompt {name}: not found")))?;
        let template = file
            .variants
            .get(variant)
            .ok_or_else(|| IntakeError::ai(format!("prompt {name}/{variant}: unknown variant")))?;
        ...
        tt.add_template(&label, &template.template)
            .map_err(|e| IntakeError::ai(format!("prompt {label}: bad template: {e}")))?;
        tt.render(&label, params)
            .map_err(|e| IntakeError::ai(format!("prompt {label}: render failed: {e}")))
    }
...
static PROMPTS: Lazy<Registry> =
    Lazy::new(|| Registry::new(&[("parse", include_str!("../prompts/parse.toml"))]));

/// Process-wide catalogue of intake prompts.
pub struct PromptRegistry;

impl PromptRegistry {
    /// Render `name` / `variant` against `params`.
    pub fn load<P: Serialize>(name: &str, variant: &str, params: &P) -> Result<String, IntakeError> {
