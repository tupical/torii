//! Concrete AI provider for torii-server: a minimal OpenAI Responses API
//! client implementing the lib's [`AiProvider`] seam.
//!
...
use torii::{AiError, AiOutput, AiProvider, AiRequest, ToolCall};
...
mod tests {
    ...
    fn build_body_minimal_and_with_tools() {
        ...
        let req = AiRequest {
            ...
            tools: vec![json!({"type": "function", "name": "create_task"})],
    ...
    fn parse_outputs_message_and_function_call() {
        let body = json!({"output": [
            ...
            {"type": "function_call", "name": "create_task", "arguments": "{\"title\":\"x\"}"}
        ]});
        let out = parse_outputs(&body).unwrap();
        assert!(matches!(&out[0], AiOutput::Text(t) if t == "hi"));
        assert!(matches!(&out[1], AiOutput::ToolCall(tc) if tc.name == "create_task"));
    }
